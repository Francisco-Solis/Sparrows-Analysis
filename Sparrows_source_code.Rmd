...
a.  Load in Sparrows dataset and display the first 5 rows of data. 
b.  Create scatterplot of weight versus wing length with a fitted regression line. 
c.  Fit a simple linear regression and display the summary output. 
d. `par(mfrow=c(1,2))` to display the following two diagnostic plots side-by-side: 
    - the residual vs fitted 
    - Normal Q-Q 
e. `par(mfrow=c(1,2))` to display the following two diagnostic plots side-by-side: 
    - Cook's distance  
...
# (a) Data structure and the first 5 rows
data("Sparrows")
str(Sparrows)
head(Sparrows,5)


# (b) ggplot to form the scatterplot
library(ggplot2)
